<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('My Income') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="">
                        <a class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded "
                            href="{{ route('income.create') }}">Add income</a>
                        <div class="mt-4 pt-4  text-center">
                            <table
                                class="items-center w-full text_center border border-gray-600 rounded-lg border-collapse bg-gray-900 text-white divide-y divide-gray-700">
                                <thead class="bg-gray-800 text-white uppercase text-sm font-semibold">
                                    <tr class="px-4 py-3">
                                        <td>#</td>
                                        <td>amount</td>
                                        <td>date & time</td>
                                        <td>action</td>
                                    </tr>
                                    <tr></tr>
                                </thead>
                                <tbody>
                                    @forelse ($incomes as $income)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ number_format($income->amount) }}</td>
                                            <td>{{ $income->date_time }}</td>
                                            <td class="flex justify-center items-center space-x-2 ">
                                                
                                                <a href="{{ route('income.show', $income->id) }}">
                                                    <img
                                                        src="data:image/png;base64," />
                                                </a>
                                                <a href="{{ route('income.edit', $income->id) }}">
                                                    <img
                                                        src="data:image/png;base64," />
                                                </a>
                                                
                                                    <button type="submit">
                                                        <a>
                                                            <img
                                                                src="data:image/png;base64," />
                                                        </a>
                                                    </button>
                                                </form>
                                            </td>
                                        </tr>
                                    @empty
                                    @endforelse
                                </tbody>
                                <tfoot></tfoot>
                            </table>




                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>
