                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="">
                        <form method="post" action="{{ route('users.update', $user->id)}}">
                            ...
                            <input value="{{ old('name',$user->name) }}" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-80" id="name" type="text" name="name" required="required" autofocus="autofocus" autocomplete="username">
                            <label class="mt-3 pt-3 block font-medium text-sm text-gray-700 dark:text-gray-300" for="email">
                                Email
                            </label>
                            <input value="{{ old('email',$user->email) }}" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-80" id="email" type="email" name="email" required="required" autofocus="autofocus" autocomplete="username">
